...
function ai_attack(attacker, attacked) {
  ...
  if (getHealth(attacked) < 1) {
    attacked_card_effects = attacked.children[4].innerHTML.split(':');
    if (attacked_card_effects[0] == 'legado') effect_dict[attacked_card_effects[1]]('computer', attacked.children[5].innerHTML);
    attacked.remove();
  }
  if (getHealth(attacker) < 1) {
    attacker_card_effects = attacker.children[4].innerHTML.split(':');
    if (attacker_card_effects[0] == 'legado') effect_dict[attacker_card_effects[1]]('computer', attacker.children[5].innerHTML);
    attacker.remove();
  }
}
...
function ai_attack_move(numOfOpponentCards) {
  ...
  for (let i = 0; i < numOfOpponentCards; i++) {
    ...
    if (maxPlayerAttack == null) {
      ...
      if (getHealth(maxPlayerAttack) <= getAttack(maxOpponentAttack) || getHealth(maxPlayerAttack) <= sumOfAttack) ai_attack(maxOpponentAttack, maxPlayerAttack);
