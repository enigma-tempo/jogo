...
class MinionCard {
  ...
  getPlayerCardsInHandHTML() {
    const rarity_colors = { Comum: '#b5b5b5', Incomum: '#31c21d', Raro: '#6689bd', Lendária: 'rgb(255, 115, 0)', Épica: 'rgb(114, 0, 180)' };
    ...
    subClassInHand.classList.add('category');
...
function freshDeck(deck_id) {
  ...
  cards.forEach((element) => {
    ...
    let effect = element['effect']['effect'];
    let params = element['params'];
    let posture = '';
    let type = element['type']['name'];
    let acting = element['acting']['name'];
    let category = element['category']['name'];
    let card = new MinionCard(attack, health, mana, info, imageString, name, rarity, effect, params, type, acting, category);
    listCards.push(card);
  });
  return listCards;
}
